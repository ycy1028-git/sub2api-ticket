export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTicketEnabled: 'Codex Turn State 打票总开关',
        codexTicketEnabledDesc:
          '关闭后不打票、不注入 x-codex-turn-state，按原链路转发。开启后后台按账号类型/账号策略获取目标门票，并在业务请求中覆盖该头。账号级目标长度和缺票调度策略请到「账号 → 编辑 → Codex Turn State 门票」设置。',
        codexTicketHarvestProxy: 'Codex Turn State 打票代理',
        codexTicketHarvestProxyDesc:
          '仅在门票功能开启时用于获取目标 State，保存后后续探测会使用新代理，无需重启。日常业务仍走账号自己的住宅代理。填写完整代理 URL（http 或 socks5h，含用户名和密码）。代理服务商需自行负责出口 IP 轮换。留空并保存表示不改已保存的值。',
        ...
        codexTicketMissRetrySeconds: '普通失败重试间隔（秒）',
        codexTicketMissRetrySecondsDesc:
          '普通探测失败或返回非目标长度后的重试间隔。默认 1800 秒（30 分钟）。',
        codexTicketRateLimitRetrySeconds: '429 限流重试间隔（秒）',
        codexTicketRateLimitRetrySecondsDesc:
          '收到上游 429 后的重试间隔。配额明确耗尽时仍会等到实际窗口重置。默认 3600 秒（1 小时）。',
        codexTicketTargetMode: '目标票模式',
        codexTicketTargetAuto: '自动按账号类型',
        codexTicketTargetManual: '手动长度',
        codexTicketTargetLength: '目标长度',
        codexTicketMissingPolicy: '未拿到票时',
        codexTicketMissingPause: '暂停该模型',
        codexTicketMissingAllow: '继续请求不注入',
        codexTicketAstra: 'astra 模型策略',
        codexTicketSol: 'sol 模型策略',
